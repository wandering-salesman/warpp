...
let video_call_progress = document.getElementsByClassName('video-call-progress')[0];
...
lang_option.addEventListener("change", (e)=> {
    console.log("laguage switched to: " + e.target.value);
...
socketForClient.on("video-signal", (data)=> {
    ...
    if(!nav_avchat_tab.children[1] && !nav_avchat_tab.classList.contains('active'))
...
.catch(error=> { console.error("Error in accessing video/audio: "+error)
})
...
const toggleNotification= (element, add)=> {
    if(add){
        element.insertAdjacentHTML("beforeend", "<span class='badge badge-warning'>!</span>")
    } else {
...
compile_run_btn.addEventListener('click', ()=> {
    ...
    fetch('/compileRun', {
        ...
        if(compile_result.errorType) {
            code_output.value = code_output.value+ "\nError: "+ compile_result.errorType;
        } 
        setTimeout( ()=> {
...
nav_chat_tab.addEventListener('click', ()=> {
    if(nav_chat_tab.children[1])
        toggleNotification(nav_chat_tab, false);
})

nav_avchat_tab.addEventListener('click', ()=> {
    if(nav_avchat_tab.children[1])
        toggleNotification(nav_avchat_tab, false);
})
