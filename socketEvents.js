...
io.on('connection', function(socket) {
    ...
    socket.on('typing', function(data) {  //recieving code, line & repo_id, ch
      console.log(`Broadcasting from sid: ${socket.id} sender_uid: ${data.uid} to repo: ${data.repo_id}`)
      const typeData = {
        code: data.code,
        line: data.line,
        ch: data.ch
    ...
    socket.on("createRepo", (data)=> { //recieving repo_id & c_uid
      socket.join(data.repo_id);
      User.findOneAndUpdate({uid: data.c_uid}, {
    ...
    socket.on("joinRepo", (data)=> {  //recieving uid and repo_id
      console.log(`sid: ${socket.id}, uid: ${data.uid} is joining ${data.repo_id}`)
      socket.join(data.repo_id);
      //inform the "other" that "this one" has joined, with img_url, name
      Repo.findOne({repo_uid: data.repo_id})
        .then( (repoData)=> {
    ...
    socket.on("video-signal", (data)=> { //recieving video_id, repo_id
    ...
    socket.on("disconnect", ()=> {
      ...
      User.findOne({socket_id: socket.id}). then( (UserData)=>{
        socket.broadcast.to(UserData.active_repo_id).emit("userLeft", {
          name: UserData.name
        })
      }).catch( e=> console.log("cannot emit leftRepo "+e));

      User.findOneAndUpdate({socket_id: socket.id}, { $unset: {socket_id: "", active_repo_id: ""}})
